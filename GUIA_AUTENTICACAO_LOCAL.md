...
5. **Histórico de Auditoria**: Cada transição no ciclo de vida do convite (geração, visualização, aceite, revogação e expiração) é registrada automaticamente com data, IP e identificador do usuário, permitindo filtragem avançada por ação ou termo no painel administrativo.

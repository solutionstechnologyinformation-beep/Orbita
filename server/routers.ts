  ...
  weeklyBackup: router({
    get: companyAdminProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (companyId == null) throw new TRPCError({ code: "BAD_REQUEST", message: "Selecione uma empresa para configurar o backup." });
        if (ctx.user.role === "company_admin" && ctx.user.companyId !== companyId) throw new TRPCError({ code: "FORBIDDEN", message: "Você não pode acessar o agendamento de outra empresa." });
        const db = await getDb();
        return (await db.select().from(backupSchedules).where(eq(backupSchedules.companyId, companyId)).limit(1))[0] ?? null;
      }),
    save: companyAdminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const db = await getDb();
        const cron = buildWeeklyBackupCron(input.dayOfWeek, input.hourUtc, input.minuteUtc);
        const nextExecutionAt = calculateNextWeeklyBackup(input.dayOfWeek, input.hourUtc, input.minuteUtc);
        const sessionToken = parseCookie(ctx.req.headers.cookie ?? "")[COOKIE_NAME] ?? "";
        const existing = (await db.select().from(backupSchedules).where(eq(backupSchedules.companyId, companyId)).limit(1))[0];
        let taskUid = existing?.scheduleCronTaskUid ?? null;
        let nextFromHeartbeat = nextExecutionAt;
        if (taskUid) {
          const result = await updateHeartbeatJob(taskUid, { cron, path: WEEKLY_BACKUP_PATH, payload: {}, description: `Backup semanal da empresa #${companyId}`, enable: input.isEnabled }, sessionToken);
          if (result.nextExecutionAt) nextFromHeartbeat = new Date(result.nextExecutionAt);
        } else {
          const job = await createHeartbeatJob({ name: `orbita-company-${companyId}-weekly-backup`, cron, path: WEEKLY_BACKUP_PATH, payload: {}, description: `Backup semanal da empresa #${companyId}` }, sessionToken);
          taskUid = job.taskUid;
        ...
        if (existing) await db.update(backupSchedules).set(values).where(eq(backupSchedules.id, existing.id));
        else await db.insert(backupSchedules).values(values);
        return (await db.select().from(backupSchedules).where(eq(backupSchedules.companyId, companyId)).limit(1))[0] ?? values;
      }),
    toggle: companyAdminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const db = await getDb();
        const existing = (await db.select().from(backupSchedules).where(eq(backupSchedules.companyId, companyId)).limit(1))[0];
        if (!existing?.scheduleCronTaskUid) throw new TRPCError({ code: "NOT_FOUND", message: "Nenhum backup semanal configurado para esta empresa." });
        const sessionToken = parseCookie(ctx.req.headers.cookie ?? "")[COOKIE_NAME] ?? "";
        await updateHeartbeatJob(existing.scheduleCronTaskUid, { enable: input.enabled }, sessionToken);
        await db.update(backupSchedules).set({ isEnabled: input.enabled }).where(eq(backupSchedules.id, existing.id));
    ...
    remove: companyAdminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (existing) await db.delete(backupSchedules).where(eq(backupSchedules.id, existing.id));
        return { success: true };
      }),
  }),

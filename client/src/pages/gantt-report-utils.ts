...
export type ReportScale = "compact" | "standard" | "large";
...
export function buildVisualGanttReportHtml(options: ReportOptions) {
  ...
  const scaleValue = scale === "compact" ? 0.84 : scale === "large" ? 1.06 : 1;
  const leftWidth = 280;
  const rowHeight = 46;
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .report{width:calc(var(--left) + var(--chart));margin:0 auto;background:#fff;border:1px solid #dce4ee;border-radius:16px;padding:26px 28px 20px;box-shadow:0 12px 35px rgba(15,23,42,.08);zoom:${scaleValue}}
  ...
  @media print{body{padding:0;background:#fff}.report{width:calc(var(--left) + var(--chart));margin:0;border:0;box-shadow:none;padding:0}.chart-shell{border-color:#cbd5e1}.task-bar,.milestone{print-color-adjust:exact;-webkit-print-color-adjust:exact}.status-badge{print-color-adjust:exact}}
  @media(max-width:900px){body{padding:12px;overflow-x:auto}.report{margin:0;padding:16px}}
</style>
</head>
<body>
  <main class="report">
    ${buildOrbitaReportHeaderHtml({ title: options.title, subtitle: "Gantt Chart — Linha do tempo de atividades", meta: `${months.length} meses em visão consolidada\nGerado em ${generatedAt}\n${formatDate(range.start)} → ${formatDate(addDays(range.end, -1))}`, logoUrl: options.logoUrl })}
